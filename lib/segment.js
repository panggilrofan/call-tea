...
const Hoek = require('@panggilrofan/hoek');
...
internals.Segment.prototype.add = function(segments, record) {
    ...
    */
    ...
    if (isLiteral) {
        ...
        if (isEdge) {            Hoek.assert(!this._param._edge, 'New route', record.path, 'conflicts with existing', this._param._edge?.record.path);
